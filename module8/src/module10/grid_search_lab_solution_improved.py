...
# hacemos predicciones usando el modelo no optimizado
train_predictions_ori = clf.predict(X_train);
test_predictions_ori = clf.predict(X_test);

f1_score_training_ori = f1_score(train_predictions_ori, y_train);
f1_score_test_ori = f1_score(test_predictions_ori, y_test);
#Mostramos el modelo
plot_model(X, y, clf);
print("The training F1 Score is (No optimizado):", f1_score_training_ori)
print("The testing F1 Score is (No optimizado):", f1_score_test_ori)

# hasta aquihay overfitting


###################
# Creamos un listado de parametros
#parameters = {'kernel':['poly', 'rbf'],'C':[0.1, 1, 10]};
parameters = {'max_depth':[2,4,6,8,10], 'min_samples_leaf':[2,4,8,10], 'min_samples_split':[2,4,6,8,10]};
clf = DecisionTreeClassifier(random_state=42);
